...
interface PassphraseFormProps {
  onVerified: () => void;
  // Omitting `location` triggers the API's legacy SECRET_OFFICE_PHRASE
  // fallback instead of the per-location pool.
  location?: LocationSlug;
}

const WORD_COUNT = 12;

export function PassphraseForm({ onVerified, location }: PassphraseFormProps) {
  const [words, setWords] = useState<string[]>(Array(WORD_COUNT).fill(''));
  const [locked, setLocked] = useState<boolean[]>(
    Array(WORD_COUNT).fill(false),
  );
    ...
    async (word: string, index: number) => {
      try {
        const res = await fetch('/api/verify-passphrase', {
          ...
          body: JSON.stringify({ word, index, location }),
        });
  ...
  const handlePaste = useCallback(
    async (e: React.ClipboardEvent, index: number) => {
